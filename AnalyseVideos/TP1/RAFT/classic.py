...
def compute_optical_flow_lk(images_path, flo_path):
    ...
    # Liste pour stocker les flux optiques
    optical_flows = []

    # Lire la première image et la convertir en niveaux de gris
    prev_img = cv2.imread(images[0], cv2.IMREAD_GRAYSCALE)
    for i in range(1, len(images)):
        # Lire la prochaine image et la convertir en niveaux de gris
        next_img = cv2.imread(images[i], cv2.IMREAD_GRAYSCALE)

        # Calculer le flux optique avec Lucas-Kanade
        ...
        # Ajouter le flux optique à la liste
...
def demo(args):
    """Run the optical flow demo with Lucas-Kanade."""
    flows = compute_optical_flow_lk(args.path, args.path_flo)
    if flows:
        print(f"Computed optical flow for {len(flows)} pairs of images.")
