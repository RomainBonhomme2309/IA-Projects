...
class EntrepotWorldMDP:
    def __init__(
        ...
        width: int,
        number_of_holes: int,
        number_of_unclean_areas: int,
    ):
        self.height = height
        self.width = width
        self.number_of_holes = number_of_holes
        self.number_of_unclean_areas = number_of_unclean_areas

        # Create all possible states (position + unclean_areas)
        self.states = set(
            (i, j, tuple(combination))
            for i in range(height)
            for j in range(width)
            for combination in itertools.product(
                [True, False], repeat=number_of_unclean_areas
            )
        )
        ...
        self.bad_states = [
            ...
            for tpl in itertools.product([True, False], repeat=number_of_unclean_areas)
        ]
        ...
        # Define the unclean_area states
        ...
        self.unclean_area_states = [
            x[:2]
            for x in random.sample(unique_available_states, number_of_unclean_areas)
        ]
    ...
    def is_new_unclean_area_state(self, state, new_state):
        for i in range(len(state[2])):
            if not state[2][i] and new_state[2][i]:
                unclean_area_pos = self.unclean_area_states[i]
                if new_state[:2] == unclean_area_pos:
                    return True
        return False
    ...
    def take_action(self, state, action):
        ...
        for i in range(len(self.unclean_area_states)):
            if new_state_pos == self.unclean_area_states[i]:
                new_state_unclean_areas = list(state[2])
                new_state_unclean_areas[i] = True
                new_state = (
                    new_state_pos[0],
                    new_state_pos[1],
                    tuple(new_state_unclean_areas),
                )
        if new_state not in self.states or new_state in self.bad_states:
            new_state = state
    ...
    def print_board(self):
        ...
        for i in range(self.height):
            ...
            for j in range(self.width):
                ...
                elif (i, j) in self.unclean_area_states:
                    for k in range(len(self.unclean_area_states)):
                        if (i, j) == self.unclean_area_states[k]:
